...
function calculateSalesTax(priceWithoutTax) {
  const saleTax = priceWithoutTax * 0.2;
  const priceWithTax = priceWithoutTax + saleTax;
  return priceWithTax;
}
...
function formatCurrency(priceUnformated) {
  const priceWithTwoDecimals =
    "£" + calculateSalesTax(priceUnformated).toFixed(2);
